...
In R, the entire audit is one pipe. The test set has columns `is_minority`, `truth` (actual), and `pred_class` (model output):
...
`group_by` slices the data; `yardstick` runs the metrics once per slice. Add `precision`, `recall`, `roc_auc`, etc. the same way.

--

Same model, very different per-group experience: accuracy drops 9 points and Cohen's kappa is nearly halved for the minority group.
